...
(defun finalize-boot-sector (compiled-section target)
  (let* ((bytes (compiled-section-bytes compiled-section))
         (sector-size (bios-target-sector-size target))
         (limit (- sector-size 2)))
    ...
    (let ((buffer (make-zero-padded-copy bytes limit)))
      (append-octet buffer #x55)
      (append-octet buffer #xAA)
      buffer)))

(defun compute-layouts (compiled-sections target)
  (let ((current-lba 1)
        (sector-size (bios-target-sector-size target)))
...
(defun append-padded-section (buffer bytes target)
  (let* ((sector-size (bios-target-sector-size target))
...
(defun build-image (designator)
  (let* ((spec (normalize-image-spec designator))
    ...
    (let* ((compiled-payload (mapcar (lambda (section)
                                     ...
                                     payload))
           (layouts (compute-layouts compiled-payload target))
           (compiled-boot (compile-section-spec boot target layouts))
           (buffer (make-octet-buffer)))
      (append-octets buffer (finalize-boot-sector compiled-boot target))
      (dolist (compiled compiled-payload)
        (append-padded-section buffer (compiled-section-bytes compiled) target))
