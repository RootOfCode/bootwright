...
(defun emit-pic-remap (environment args)
  (let* ((state (compile-environment-state environment))
         (args (resolve-operands environment args))
         (master-offset (getf args :master-offset #x20))
         (slave-offset (getf args :slave-offset #x28))
    ...
    (flet ((write-port (port value)
      ...
      (write-port #x20 icw1)
      (write-port #xA0 icw1)
      (write-port #x21 master-offset)
      (write-port #xA1 slave-offset)
      (write-port #x21 4)
      (write-port #xA1 2)
      (write-port #x21 icw4)
      (write-port #xA1 icw4)
      (encode-instruction state 'mov (list 'al master-mask))
      (encode-instruction state 'out '(#x21 al))
      (encode-instruction state 'mov (list 'al slave-mask))
      (encode-instruction state 'out '(#xA1 al)))))
...
(defun emit-pic-eoi (environment args)
  (let* ((state (compile-environment-state environment))
    ...
    (when (and irq (>= irq 8))
      (encode-instruction state 'out '(#xA0 al)))
    (encode-instruction state 'out '(#x20 al))))
...
(defmacro defimage (name options &body sections)
  (let ((target (getf options :target)))
    (unless target
      (error "DEFIMAGE requires a :TARGET option."))
    `(defparameter ,name
